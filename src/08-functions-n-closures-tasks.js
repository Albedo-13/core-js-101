...
function getPolynom(...args) {
  ...
  function calcPolynom(x = 1) {
    let result = 0;
    for (let i = 0; i < argLength; i += 1) {
      result += args[argLength - 1 - i] * (x ** i);
    }
    return result;
  }
  ...
  return () => {
    for (let i = 1; i <= attempts; i += 1) {
      try {
        ...
        if (i + 1 > attempts) {
          throw new Error('Finish all attempts (inner)');
        }
      }
    }
    throw new Error('Finish all attempts (outer)');
  };
}
...
function logger(func, logFunc) {
  return (arg1, ...rest) => {
    let logValue = `${func.name}(${JSON.stringify(arg1)},${rest})`;
    if (rest.length === 0) {
      logValue = logValue.replace(',', '');
    }

    logFunc(`${logValue} starts`);
    const funcResult = func(arg1, ...rest);
    logFunc(`${logValue} ends`);
    return funcResult;
  };
}
...
function getIdGeneratorFunction(startFrom) {
  let incrementIndex = startFrom - 1;
  return () => {
    incrementIndex += 1;
